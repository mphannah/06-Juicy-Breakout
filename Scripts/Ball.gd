...
onready var Wall_Hit = get_node("/root/Game/Wall_Hit")
...
func _ready():
	...
	_start_size = $ColorRect.rect_size
...
func _process(delta):
	...
	if _size > 0:
		_decay_size(delta)
		_apply_growth()
	if _size == 0 or $ColorRect.rect_size != _start_size:
		$ColorRect.rect_size = _start_size
...
func add_growth(amount):
	_size = min(_size + amount, 1)

func _apply_growth():
	var b = min(1, _size)
	$ColorRect.rect_size = _start_size * Vector2(1,1)

func _decay_size(delta):
	var change = _size_decay * delta
	_size = max(_size - change, 0)
